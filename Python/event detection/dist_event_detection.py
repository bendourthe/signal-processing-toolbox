...
def dist_event_detection(time, signal, side=None, mode=None, manual_window=None, auto_window=None, plot=None):
    '''
    Detect takeoff and landing during a distance single legged hop trial.
    Input:
    ...
    if mode == 'manual':
        ...
        #       Landing
        # find actual local minima closest to the selected point
        landing = np.argmin(signal[idx[1]-manual_window:idx[1]+manual_window]) + idx[1]-manual_window
